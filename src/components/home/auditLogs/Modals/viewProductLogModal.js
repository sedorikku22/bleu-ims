...
import './viewProductLogModal.css';

const ViewProductLogModal = ({ log, onClose }) => {
    ...
    const generatePDF = async () => {
        ...
        doc.text('PRODUCT LOG REPORT', 105, 15, { align: 'center' });
        ...
        const productBody = [
            ...
            ['Description', log.ProductDescription || 'N/A'],
            ['Price', `₱${parseFloat(log.ProductPrice).toFixed(2)}`],
            ['Status', log.Status || 'N/A']
        ];

        if (log.ProductSizes && log.ProductSizes.length > 0) {
            productBody.push(['Sizes', log.ProductSizes.join(', ')]);
        }
        ...
        doc.save(`Product_Log_${log.ProductID}_${Date.now()}.pdf`);
    };
    ...
    const renderChanges = () => {
        ...
        if (changes.length === 0) {
            return <p className="no-changes">No changes detected</p>;
        }
                ...
                {changes.map(([field, oldVal, newVal], index) => (
                    <div key={index} className="change-item">
                        ...
                        <div className="change-values">
                            <div className="change-value-box old-value">
                                <span className="change-label">Old:</span>
                                <span className="change-text">{oldVal}</span>
                            </div>
                            <div className="change-arrow">→</div>
                            <div className="change-value-box new-value">
                                <span className="change-label">New:</span>
                                <span className="change-text">{newVal}</span>
                            </div>
                        </div>
                    </div>
                ))}
            ...
            <div className="modal-content" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="modal-body">
                    <div className="log-section">
                        ...
                        <div className="log-details-grid">
                            <div className="log-detail-item">
                                <span className="log-label">Block ID:</span>
                                <span className="log-value">{log.block_id}</span>
                            </div>
                            <div className="log-detail-item">
                                <span className="log-label">Timestamp:</span>
                                <span className="log-value">{formatTimestamp(log.timestamp)}</span>
                            </div>
                            <div className="log-detail-item">
                                <span className="log-label">Action:</span>
                                <span className={`log-value action-badge ${log.action.toLowerCase()}`}>
                                    {log.action}
                                </span>
                            </div>
                            <div className="log-detail-item">
                            ...
                            <div className="log-detail-item">
                                <span className="log-label">Hash:</span>
                                <a className="log-value" href="https://sepolia.etherscan.io/address/0xee04ec86745cdad35e286d95cee333f81dae72e4" target="_blank" rel="noopener noreferrer">{log.tx_hash || 'N/A'}</a>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="log-section">
                        ...
                        <div className="log-details-grid">
                            <div className="log-detail-item">
                                <span className="log-label">Product ID:</span>
                                <span className="log-value">{log.ProductID}</span>
                            </div>
                            <div className="log-detail-item">
                                <span className="log-label">Product Name:</span>
                                <span className="log-value">{log.ProductName}</span>
                            </div>
                            <div className="log-detail-item">
                                <span className="log-label">Product Type ID:</span>
                                <span className="log-value">{log.ProductTypeID}</span>
                            </div>
                            <div className="log-detail-item">
                                <span className="log-label">Category:</span>
                                <span className="log-value">{log.ProductCategory}</span>
                            </div>
                            <div className="log-detail-item full-width">
                                <span className="log-label">Description:</span>
                                <span className="log-value">{log.ProductDescription || 'N/A'}</span>
                            </div>
                            <div className="log-detail-item">
                                <span className="log-label">Price:</span>
                                <span className="log-value">₱{parseFloat(log.ProductPrice).toFixed(2)}</span>
                            </div>
                            <div className="log-detail-item">
                                <span className="log-label">Status:</span>
                                <span className="log-value">{log.Status || 'N/A'}</span>
                            </div>
                            {log.ProductSizes && log.ProductSizes.length > 0 && (
                                <div className="log-detail-item full-width">
                                    <span className="log-label">Sizes:</span>
                                    <span className="log-value">{log.ProductSizes.join(', ')}</span>
                                </div>
                            )}
                        </div>
                    </div>
                ...
                <div className="modal-footer">
                    <button className="modal-btn-export" onClick={generatePDF}>
...
export default ViewProductLogModal;